...
- **Implicit Transactions**: Done via `DbProvider` utilizing `AsyncLocalStorage` (`transactionStorage`). Wrap multi-step writes in `this.dbProvider.transaction(async () => { ... })` and always query through `this.dbProvider.client` (which resolves to the active tx), avoiding passing around `tx` parameters. Never call the raw Drizzle `db.transaction(tx => ...)` directly — that bypasses the AsyncLocalStorage propagation.
